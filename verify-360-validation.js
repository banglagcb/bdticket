...
function verifyValidationSystem() {
  ...
  try {
    const validationContent = fs.readFileSync('client/lib/validation.ts', 'utf8');
  ...
  try {
    const adminBuyingContent = fs.readFileSync('client/pages/AdminBuying.tsx', 'utf8');
    ...
    adminFeatures.forEach(feature => {
      const hasFeature = adminBuyingContent.includes(feature) || 
                         adminBuyingContent.toLowerCase().includes(feature.toLowerCase());
      const status = hasFeature ? '✅' : '❌';
      console.log(`   ${status} ${feature}`);
      results.push({ type: 'admin-feature', name: feature, passed: hasFeature });
      if (!hasFeature) allPassed = false;
    });
  } catch (error) {
  ...
  try {
    const bookingsContent = fs.readFileSync('client/pages/Bookings.tsx', 'utf8');
    ...
    bookingFeatures.forEach(feature => {
      const hasFeature = bookingsContent.includes(feature) || 
                         bookingsContent.toLowerCase().includes(feature.toLowerCase());
      const status = hasFeature ? '✅' : '❌';
      console.log(`   ${status} ${feature}`);
      results.push({ type: 'booking-feature', name: feature, passed: hasFeature });
      if (!hasFeature) allPassed = false;
    });
  } catch (error) {
  ...
  try {
    const allContent = requiredFiles.map(file => {
      try {
