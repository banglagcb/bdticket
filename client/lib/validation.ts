...
// 💰 Price Validation
export const validatePrice = (price: number, min: number = 1000, max: number = 500000): ValidationResult => {
  const errors: string[] = [];
  
  if (!price || price <= 0) {
    errors.push("মূল্য ০ এর চেয়ে বেশি হতে হবে / Price must be greater than 0");
  } else if (price < min) {
    errors.push(`মূল্য কমপক্ষে ৳${min.toLocaleString()} হতে হবে / Price must be at least ৳${min.toLocaleString()}`);
  } else if (price > max) {
    errors.push(`মূল্য সর্বোচ্চ ৳${max.toLocaleString()} হতে পারে / Price cannot exceed ৳${max.toLocaleString()}`);
  }
...
export const validateFlightDate = (dateString: string): ValidationResult => {
  ...
  if (flightDate < today) {
    ...
    errors.push("১ বছরের মধ্যে তারিখ নির্বাচন করুন / Please select date within 1 year");
  }
  ...
  if (flightDate <= threeDaysLater) {
    warnings.push("খুব শীঘ্রই ফ্লাইট - তাড়াহুড়ো বিক্রয় প্রয়োজন / Very soon flight - rush sale required");
  }
...
export const validateFlightTime = (timeString: string): ValidationResult => {
  ...
  if (!timeString) {
    ...
    if (!timeRegex.test(timeString)) {
      errors.push("সঠিক সময় ফরম্যাট ব্যবহার করুন (HH:MM) / Please use correct time format (HH:MM)");
    }
  }
...
// 🎫 Quantity Validation
export const validateTicketQuantity = (quantity: number, maxQuantity: number = 1000): ValidationResult => {
  const errors: string[] = [];
  const warnings: string[] = [];
  
  if (!quantity || quantity <= 0) {
    errors.push("টিকেটের সংখ্যা ০ এর চেয়ে বেশি হতে হবে / Quantity must be greater than 0");
  } else if (quantity > maxQuantity) {
    errors.push(`একবারে সর্বোচ্চ ${maxQuantity} টিকেট ক্রয় করা যাবে / Maximum ${maxQuantity} tickets can be purchased at once`);
  }
  
  if (quantity > 500) {
    warnings.push("বড় পরিমাণের টিকেট - বিক্রয় পরিকল্পনা নিশ্চিত করুন / Large quantity - ensure sales plan");
  }
...
export const validateAgentInfo = (name: string, contact: string, address?: string): ValidationResult => {
  const errors: string[] = [];
  
  if (!name || name.trim().length < 3) {
    errors.push("এজেন্টের নাম কমপক্ষে ৩ অক্ষর হতে হবে / Agent name must be at least 3 characters");
  }
  
  if (!contact || !validateBangladeshiPhone(contact)) {
    errors.push("সঠিক বাংলাদেশি মোবাইল নম্বর দিন / Please provide valid Bangladeshi mobile number");
  }
  
  if (address && address.trim().length > 0 && address.trim().length < 10) {
    errors.push("ঠিকানা কমপক্ষে ১০ অক্ষর হতে হবে / Address must be at least 10 characters");
  }
...
export const calculateFinancials = (buyingPrice: number, quantity: number, markupPercentage: number = 20): FinancialCalculation => {
  const totalCost = buyingPrice * quantity;
  const estimatedSellingPrice = Math.round(buyingPrice * (1 + markupPercentage / 100));
  const estimatedRevenue = estimatedSellingPrice * quantity;
  const estimatedProfit = estimatedRevenue - totalCost;
  const profitMargin = totalCost > 0 ? ((estimatedProfit / totalCost) * 100).toFixed(1) : "0";
  ...
  if (totalCost > 5000000 || parseFloat(profitMargin) < 10 || quantity > 500) {
    riskLevel = 'high';
  } else if (totalCost > 1000000 || parseFloat(profitMargin) < 15 || quantity > 200) {
    riskLevel = 'medium';
  }
...
export const validateStatusTransition = (currentStatus: string, newStatus: string): ValidationResult => {
  const errors: string[] = [];
...
export const validatePassengerInfo = (name: string, passport: string, phone: string, email?: string): ValidationResult => {
  const errors: string[] = [];
  
  if (!name || name.trim().length < 2) {
    errors.push("যাত্রীর নাম কমপক্ষে ২ অক্ষর হতে হবে / Passenger name must be at least 2 characters");
  }
  
  if (!passport || !validatePassportNumber(passport)) {
    errors.push("সঠিক পাসপোর্ট নম্বর দিন (যেমন: AB1234567) / Please provide valid passport number (e.g., AB1234567)");
  }
...
export const validateTicketBatch = (batchData: {
  ...
  if (!batchData.country) allErrors.push("দেশ নির্বাচন করা আবশ্যক / Country selection is required");
  if (!batchData.airline) allErrors.push("এয়ারলাইন নির্বাচন করা আবশ্যক / Airline selection is required");
  ...
  // Agent validation
  const agentValidation = validateAgentInfo(batchData.agentName, batchData.agentContact, batchData.agentAddress);
  ...
  if (totalCost > 50000000) { // 5 crore limit
    allErrors.push("মোট খরচ ৫ কোটি টাকার বেশি হতে পারে না / Total cost cannot exceed ৳5 crore");
  }
...
// 📋 Audit Log Generator
export const generateAuditLog = (action: string, data: any, user: any): string => {
...
export const validateSecurityPermissions = (user: any, action: string): ValidationResult => {
  const errors: string[] = [];
  ...
  if (requiredRoles && !requiredRoles.includes(user.role)) {
    errors.push(`এই কার্যক্রমের অনুমতি নেই / No permission for this action: ${action}`);
  }
