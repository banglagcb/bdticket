...
export default function AdminBuying() {
  ...
  const airlines = [
    'Air Arabia', 'Emirates', 'Qatar Airways', 'Saudi Airlines', 
    'Flydubai', 'Kuwait Airways', 'Oman Air', 'Gulf Air'
  ];

  const handleInputChange = (field: keyof CreateTicketBatchRequest, value: string | number) => {
    setFormData(prev => ({
      ...prev,
      [field]: value
    }));
  ...
  const filteredPurchases = pastPurchases.filter(purchase => {
    const matchesSearch = purchase.airline.toLowerCase().includes(searchTerm.toLowerCase()) ||
                         purchase.agentName.toLowerCase().includes(searchTerm.toLowerCase());
    const matchesCountry = countryFilter === 'all' || purchase.country === countryFilter;
        ...
        <div className="flex items-center justify-between">
          ...
          <div className="hidden lg:flex items-center space-x-6 luxury-card p-4 rounded-lg border-0">
            <div className="text-center">
              ...
              <p className="text-xs font-body text-foreground/60">Total Investment</p>
            </div>
            <div className="text-center">
              <p className="text-xl font-heading font-bold text-green-600 velvet-text">
                ৳{totalStats.totalProfit.toLocaleString()}
              </p>
              <p className="text-xs font-body text-foreground/60">Total Profit</p>
            </div>
            <div className="text-center">
              ...
              <CardContent>
                <form onSubmit={handleSubmit} className="space-y-6">
                  ...
                  <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
                    ...
                    <div className="space-y-2">
                      <Label className="font-body font-medium">Flight Date</Label>
                    ...
                    <div className="space-y-2">
                      <Label className="font-body font-medium">Flight Time</Label>
                      <div className="relative">
                    ...
                    <div className="space-y-2">
                      <Label className="font-body font-medium">Buying Price (Per Ticket)</Label>
                      <div className="relative">
                        <DollarSign className="absolute left-3 top-3 h-4 w-4 text-foreground/40" />
                        <Input
                          type="number"
                          value={formData.buyingPrice || ''}
                          onChange={(e) => handleInputChange('buyingPrice', parseInt(e.target.value) || 0)}
                          className="pl-10 font-body"
                          placeholder="18000"
                          required
                        />
                    ...
                    <div className="space-y-2">
                      <Label className="font-body font-medium">Total Tickets</Label>
                      <div className="relative">
                  ...
                  <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
                    <div className="space-y-2">
                      <Label className="font-body font-medium">Agent/Seller Name</Label>
                      <div className="relative">
                    ...
                    <div className="space-y-2">
                      <Label className="font-body font-medium">Agent Contact (Optional)</Label>
                      <div className="relative">
                    ...
                    <div className="space-y-2">
                      <Label className="font-body font-medium">Agent Address (Optional)</Label>
                      <div className="relative">
                  ...
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                    <div className="space-y-2">
                      <Label className="font-body font-medium">Remarks (Optional)</Label>
                      <div className="relative">
                    ...
                    <div className="space-y-2">
                      <Label className="font-body font-medium">Upload Invoice (Optional)</Label>
                  ...
                  {formData.quantity > 0 && formData.buyingPrice > 0 && (
                    ...
                    >
                      <h3 className="font-heading font-semibold velvet-text mb-2">Cost Summary</h3>
                      <div className="grid grid-cols-1 md:grid-cols-3 gap-4 text-sm font-body">
                        ...
                        <div>
                          <span className="text-foreground/70">Total tickets:</span>
                          <span className="font-semibold text-foreground ml-2">
                            {formData.quantity}
                          </span>
                        </div>
                        <div>
                          ...
                          <span className="font-semibold text-primary ml-2">
                            ৳{(formData.buyingPrice * formData.quantity).toLocaleString()}
                          </span>
                        </div>
                      </div>
                    </motion.div>
                ...
                <div className="overflow-x-auto">
                  <table className="w-full">
                    <thead className="bg-gradient-to-r from-cream-100 to-cream-200 border-b border-border/30">
                      <tr>
                        <th className="px-4 py-3 text-left font-heading font-semibold text-sm text-foreground velvet-text">Country</th>
                        <th className="px-4 py-3 text-left font-heading font-semibold text-sm text-foreground velvet-text">Airline</th>
                        <th className="px-4 py-3 text-left font-heading font-semibold text-sm text-foreground velvet-text">Flight Date</th>
                        <th className="px-4 py-3 text-left font-heading font-semibold text-sm text-foreground velvet-text">Quantity</th>
                        <th className="px-4 py-3 text-left font-heading font-semibold text-sm text-foreground velvet-text">Buying Price</th>
                        <th className="px-4 py-3 text-left font-heading font-semibold text-sm text-foreground velvet-text">Total Cost</th>
                        <th className="px-4 py-3 text-left font-heading font-semibold text-sm text-foreground velvet-text">Agent</th>
                        <th className="px-4 py-3 text-left font-heading font-semibold text-sm text-foreground velvet-text">Status</th>
                        <th className="px-4 py-3 text-left font-heading font-semibold text-sm text-foreground velvet-text">Profit</th>
                      </tr>
                    </thead>
                    <tbody>
                      {filteredPurchases.map((purchase, index) => (
                        ...
                        >
                          <td className="px-4 py-3">
                            <div className="flex items-center space-x-2">
                              <span className="text-lg">{countries.find(c => c.code === purchase.country)?.flag}</span>
                              <span className="font-body font-medium text-sm text-foreground">{purchase.country}</span>
                            </div>
                          </td>
                          <td className="px-4 py-3">
                            <div className="flex items-center space-x-2">
                              <Plane className="h-4 w-4 text-foreground/40" />
                              <span className="font-body text-sm text-foreground">{purchase.airline}</span>
                            </div>
                          </td>
                          <td className="px-4 py-3 font-body text-sm text-foreground">{purchase.flightDate}</td>
                          <td className="px-4 py-3 font-body text-sm text-foreground">{purchase.quantity}</td>
                          <td className="px-4 py-3 font-body text-sm text-foreground">৳{purchase.buyingPrice.toLocaleString()}</td>
                          <td className="px-4 py-3 font-body font-semibold text-sm text-foreground">৳{purchase.totalCost.toLocaleString()}</td>
                          <td className="px-4 py-3">
                            <div>
                              <p className="font-body font-medium text-sm text-foreground">{purchase.agentName}</p>
                              {purchase.agentContact && (
                                <p className="font-body text-xs text-foreground/60">{purchase.agentContact}</p>
                              )}
                            </div>
                          </td>
                          <td className="px-4 py-3">
