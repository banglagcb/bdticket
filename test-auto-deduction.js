...
async function createTestGroupTicket(token) {
  try {
    ...
    const returnDate = new Date(departureDate.getTime() + 10 * 24 * 60 * 60 * 1000); // 10 days later
...
async function createPassengerWithGroupTicket(token, groupTicket) {
  try {
    ...
    if (result.success) {
      ...
      console.log(`   Group Assignment: ${result.groupAssignment ? 'Yes' : 'No'}`);
      return result.data;
    } else {
...
async function checkGroupTicketStatus(token, groupTicketId) {
  try {
    ...
    const response = await fetch(`${API_BASE}/umrah/debug/group-ticket/${groupTicketId}`, {
      headers: {
        'Authorization': `Bearer ${token}`
      }
    });
    ...
    if (result.success) {
      ...
      console.log(`   Current Remaining: ${data.groupTicket.remaining_tickets}`);
      ...
      if (data.assignments.length > 0) {
        ...
        data.assignments.forEach((assignment, index) => {
          console.log(`     ${index + 1}. Passenger ID: ${assignment.passenger_id} (${assignment.passenger_type})`);
        });
      }
      
      // Check if remaining tickets is correct
      const isCorrect = data.groupTicket.remaining_tickets === data.calculatedRemaining;
      console.log(`   Status: ${isCorrect ? '✅ Correct' : '❌ Incorrect - needs fix!'}`);
...
async function testAutoDeduction() {
  ...
  if (finalStatus) {
    const ticketReduced = finalStatus.groupTicket.remaining_tickets < groupTicket.ticket_count;
    ...
    console.log(`${assignmentCreated ? '✅' : '❌'} Assignment Created: ${assignmentCreated ? 'Yes' : 'No'}`);
    console.log(`${ticketReduced ? '✅' : '❌'} Ticket Count Reduced: ${ticketReduced ? 'Yes' : 'No'}`);
    console.log(`📊 Final Status: ${groupTicket.ticket_count} → ${finalStatus.groupTicket.remaining_tickets} (${finalStatus.assignments.length} assigned)`);
    ...
    if (!success) {
      console.log('\n💡 Possible Issues:');
      if (!assignmentCreated) console.log('   • Group booking assignment not created');
      if (!ticketReduced) console.log('   • Remaining tickets count not updated');
