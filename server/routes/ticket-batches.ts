...
router.get('/', requirePermission('view_profit'), async (req: Request, res: Response) => {
  try {
    ...
    if (airline) {
      batches = batches.filter(batch => batch.airline_name.toLowerCase().includes((airline as string).toLowerCase()));
    }
    ...
    const batchesWithStats = batches.map(batch => {
      const tickets = TicketRepository.findAll().filter(ticket => ticket.batch_id === batch.id);
      const sold = tickets.filter(ticket => ticket.status === 'sold').length;
      const locked = tickets.filter(ticket => ticket.status === 'locked').length;
      ...
      const totalRevenue = soldTickets.reduce((sum, ticket) => sum + ticket.selling_price, 0);
...
router.get('/:id', requirePermission('view_profit'), async (req: Request, res: Response) => {
  try {
    const { id } = req.params;
    ...
    const tickets = TicketRepository.findAll().filter(ticket => ticket.batch_id === id);
...
router.post('/', requirePermission('create_batches'), async (req: Request, res: Response) => {
  try {
    ...
    for (let i = 1; i <= batchData.quantity; i++) {
      ...
      const airlineCode = batchData.airline === 'Air Arabia' ? 'G9' : 
                         batchData.airline === 'Emirates' ? 'EK' : 
                         batchData.airline === 'Qatar Airways' ? 'QR' :
                         batchData.airline === 'Saudi Airlines' ? 'SV' :
                         batchData.airline === 'Flydubai' ? 'FZ' : 'XX';
      ...
      const ticket = TicketRepository.create({
        ...
        aircraft: batchData.airline === 'Air Arabia' ? 'Airbus A320' : 
                 batchData.airline === 'Emirates' ? 'Boeing 777' : 
                 batchData.airline === 'Qatar Airways' ? 'Boeing 787' :
                 'Airbus A321',
...
router.put('/:id', requirePermission('edit_batches'), async (req: Request, res: Response) => {
  try {
    const { id } = req.params;
...
router.delete('/:id', requirePermission('delete_batches'), async (req: Request, res: Response) => {
  try {
    const { id } = req.params;
