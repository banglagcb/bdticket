...
import { BookingRepository, TicketRepository, ActivityLogRepository } from '../database/models';
...
router.get('/:id', async (req: Request, res: Response) => {
  try {
    ...
    const canViewAllBookings = hasPermission(req.user!.role, 'view_all_bookings');
  ...
  try {
    ...
    ActivityLogRepository.create({
      ...
      details: JSON.stringify({
        ...
        passenger_name: bookingData.passengerInfo.name,
        payment_type: bookingData.paymentType,
        amount: bookingData.paymentType === 'full' 
  ...
  try {
    ...
    if (success) {
      ...
      if (status === 'confirmed') {
        ...
        TicketRepository.updateStatus(booking.ticket_id, newTicketStatus, req.user!.id);
