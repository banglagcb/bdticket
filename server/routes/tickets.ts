...
import { TicketRepository, TicketBatchRepository, CountryRepository, ActivityLogRepository } from '../database/models';
...
router.get('/', async (req: Request, res: Response) => {
  try {
    ...
    // Apply filters
    if (country) {
      tickets = tickets.filter(ticket => ticket.batch.country_code === country);
    }
    ...
    if (airline) {
      tickets = tickets.filter(ticket => ticket.batch.airline_name.toLowerCase().includes((airline as string).toLowerCase()));
    }
    ...
    const userCanViewBuyingPrice = hasPermission(req.user!.role, 'view_buying_price');
...
router.patch('/:id/status', async (req: Request, res: Response) => {
  try {
    ...
    const success = TicketRepository.updateStatus(id, status, status === 'sold' ? req.user!.id : undefined);
...
router.get('/countries/stats', async (req: Request, res: Response) => {
  try {
    ...
    const countriesWithStats = countries.map(country => {
      const countryStats = stats.find(stat => stat.country_code === country.code);
      return {
        ...country,
