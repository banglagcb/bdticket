...
export function calculateFinancialSummary(): FinancialSummary {
  ...
  `).all() as (Ticket & { batch_buying_price: number; country_code: string; airline_name: string })[];
  ...
  `).all() as (Booking & { ticket_selling_price: number; batch_buying_price: number })[];
  ...
  const totalTicketsBought = batches.reduce((sum, batch) => sum + batch.quantity, 0);
  ...
  const inventoryUtilization = totalTicketsBought > 0 ? (totalTicketsSold / totalTicketsBought) * 100 : 0;
  const averageBuyingPrice = totalTicketsBought > 0 ? totalInvestment / totalTicketsBought : 0;
  const averageSellingPrice = totalTicketsSold > 0 ? totalRevenue / totalTicketsSold : 0;
...
export function calculateCountryFinancials(): CountryFinancials {
  ...
  const batchesByCountry = db.prepare(`
    SELECT 
      country_code,
  ...
  const availableByCountry = db.prepare(`
    SELECT 
      tb.country_code,
  ...
  batchesByCountry.forEach(batch => {
    const sales = salesByCountry.find(s => s.country_code === batch.country_code);
    const available = availableByCountry.find(a => a.country_code === batch.country_code);
 ...
 */
export function calculatePotentialProfit(ticketId: string, sellingPrice: number): number {
 ...
 */
export function calculateOptimalSellingPrice(buyingPrice: number, countryCode: string): number {
  // Get average selling price for this country
  const countryAvg = db.prepare(`
 ...
 */
export function validateBooking(ticketId: string, sellingPrice: number): {
  valid: boolean;
  error?: string;
  ticketData?: any;
} {
  ...
  if (ticket.status !== 'available') {
    return { valid: false, error: `Ticket is ${ticket.status}, not available for booking` };
  }
