...
async function testUmrahWithTransport() {
  ...
  logResult(
      ...
      : `Status ${getResponse.status}: ${getResponse.result.message || "Failed"}`,
    {
      ...
      structure: getResponse.result.data?.[0]
        ? Object.keys(getResponse.result.data[0])
        : "No data",
    },
  );

  // Test with search parameter
  const searchResponse = await testEndpoint(
    "/umrah/with-transport?search=test",
  );
  logResult(
...
async function testUmrahWithoutTransport() {
  ...
  const pendingResponse = await testEndpoint(
    "/umrah/without-transport?pending_only=true",
  );
  logResult(
...
async function testUmrahGroupTickets() {
  ...
  const withTransportGroups = await testEndpoint(
    "/umrah/group-tickets?package_type=with-transport",
  );
  logResult(
  ...
  const withoutTransportGroups = await testEndpoint(
    "/umrah/group-tickets?package_type=without-transport",
  );
  logResult(
...
async function testUmrahGroupTicketsByDates() {
  ...
  const withTransportByDates = await testEndpoint(
    "/umrah/group-tickets/by-dates/with-transport",
  );
  logResult(
      ...
      : `Status ${withTransportByDates.status}: ${withTransportByDates.result.message || "Failed"}`,
    {
      ...
      dataStructure: withTransportByDates.result.data
        ? typeof withTransportByDates.result.data
        : "No data",
      dataKeys: withTransportByDates.result.data
        ? Object.keys(withTransportByDates.result.data)
        : [],
    },
  );

  // Test GET /api/umrah/group-tickets/by-dates/without-transport
  const withoutTransportByDates = await testEndpoint(
    "/umrah/group-tickets/by-dates/without-transport",
  );
  logResult(
      ...
      : `Status ${withoutTransportByDates.status}: ${withoutTransportByDates.result.message || "Failed"}`,
    {
      ...
      dataStructure: withoutTransportByDates.result.data
        ? typeof withoutTransportByDates.result.data
        : "No data",
      dataKeys: withoutTransportByDates.result.data
        ? Object.keys(withoutTransportByDates.result.data)
        : [],
    },
  );
}
