...
try {
  ...
  for (const ticket of groupTickets) {
    ...
    if (ticket.remaining_tickets !== actualRemaining) {
      ...
      console.log(`   ✅ Fixed: ${ticket.remaining_tickets || 'NULL'} → ${actualRemaining}`);
      fixed++;
    } else {
  ...
  if (fixed > 0) {
    ...
    updatedTickets.forEach(ticket => {
      console.log(`   • ${ticket.group_name}: ${ticket.ticket_count} total, ${ticket.assigned_count} assigned, ${ticket.remaining_tickets} remaining`);
    });
  }
